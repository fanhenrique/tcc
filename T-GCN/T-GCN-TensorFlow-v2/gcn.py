...
class GCN(object):     
    ...
    def __init__(self, num_units, adj, inputs, output_dim, activation = tf.nn.tanh, 
        ...
        self._batch_size = tf.shape(input=inputs)[0]
        self._adj = []  
    ...
    def _gconv(self,scope=None):
        ####[batch, num_nodes, seq]
        inputs = self._inputs       
        inputs = tf.transpose(a=inputs, perm=[2,0,1])
#        print('0',inputs.get_shape())
        ...
        with tf.compat.v1.variable_scope(scope):
            ...
            for adj in self._adj:
                x1 = tf.sparse.sparse_dense_matmul(adj, x0)
            ...
            self.output = tf.transpose(a=self.output, perm=[1,2,0])
